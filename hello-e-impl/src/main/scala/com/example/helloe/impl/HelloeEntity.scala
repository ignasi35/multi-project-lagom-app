...
/**
  * This is an event sourced entity. It has a state, [[HelloeState]], which
  ...
  */
class HelloeEntity extends PersistentEntity {

  override type Command = HelloeCommand[_]
  override type Event = HelloeEvent
  override type State = HelloeState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: HelloeState = HelloeState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case HelloeState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        HelloeState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class HelloeState(message: String, timestamp: String)
...
object HelloeState {
  /**
    ...
    */
  implicit val format: Format[HelloeState] = Json.format
}

/**
  * This interface defines all the events that the HelloeEntity supports.
  */
sealed trait HelloeEvent extends AggregateEvent[HelloeEvent] {
  def aggregateTag: AggregateEventTag[HelloeEvent] = HelloeEvent.Tag
}

object HelloeEvent {
  val Tag: AggregateEventTag[HelloeEvent] = AggregateEventTag[HelloeEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends HelloeEvent
...
/**
  * This interface defines all the commands that the HelloeEntity supports.
  */
sealed trait HelloeCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends HelloeCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends HelloeCommand[String]
...
/**
  ...
  * application loader.
  */
object HelloeSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[HelloeState]
  )
}
