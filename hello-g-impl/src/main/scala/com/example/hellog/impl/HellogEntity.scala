...
/**
  * This is an event sourced entity. It has a state, [[HellogState]], which
  ...
  */
class HellogEntity extends PersistentEntity {

  override type Command = HellogCommand[_]
  override type Event = HellogEvent
  override type State = HellogState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: HellogState = HellogState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case HellogState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        HellogState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class HellogState(message: String, timestamp: String)
...
object HellogState {
  /**
    ...
    */
  implicit val format: Format[HellogState] = Json.format
}

/**
  * This interface defines all the events that the HellogEntity supports.
  */
sealed trait HellogEvent extends AggregateEvent[HellogEvent] {
  def aggregateTag: AggregateEventTag[HellogEvent] = HellogEvent.Tag
}

object HellogEvent {
  val Tag: AggregateEventTag[HellogEvent] = AggregateEventTag[HellogEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends HellogEvent
...
/**
  * This interface defines all the commands that the HellogEntity supports.
  */
sealed trait HellogCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends HellogCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends HellogCommand[String]
...
/**
  ...
  * application loader.
  */
object HellogSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[HellogState]
  )
}
