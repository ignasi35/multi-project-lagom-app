...
/**
  * This is an event sourced entity. It has a state, [[HellocState]], which
  ...
  */
class HellocEntity extends PersistentEntity {

  override type Command = HellocCommand[_]
  override type Event = HellocEvent
  override type State = HellocState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: HellocState = HellocState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case HellocState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        HellocState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class HellocState(message: String, timestamp: String)
...
object HellocState {
  /**
    ...
    */
  implicit val format: Format[HellocState] = Json.format
}

/**
  * This interface defines all the events that the HellocEntity supports.
  */
sealed trait HellocEvent extends AggregateEvent[HellocEvent] {
  def aggregateTag: AggregateEventTag[HellocEvent] = HellocEvent.Tag
}

object HellocEvent {
  val Tag: AggregateEventTag[HellocEvent] = AggregateEventTag[HellocEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends HellocEvent
...
/**
  * This interface defines all the commands that the HellocEntity supports.
  */
sealed trait HellocCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends HellocCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends HellocCommand[String]
...
/**
  ...
  * application loader.
  */
object HellocSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[HellocState]
  )
}
