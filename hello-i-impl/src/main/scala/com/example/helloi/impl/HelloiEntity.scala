...
/**
  * This is an event sourced entity. It has a state, [[HelloiState]], which
  ...
  */
class HelloiEntity extends PersistentEntity {

  override type Command = HelloiCommand[_]
  override type Event = HelloiEvent
  override type State = HelloiState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: HelloiState = HelloiState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case HelloiState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        HelloiState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class HelloiState(message: String, timestamp: String)
...
object HelloiState {
  /**
    ...
    */
  implicit val format: Format[HelloiState] = Json.format
}

/**
  * This interface defines all the events that the HelloiEntity supports.
  */
sealed trait HelloiEvent extends AggregateEvent[HelloiEvent] {
  def aggregateTag: AggregateEventTag[HelloiEvent] = HelloiEvent.Tag
}

object HelloiEvent {
  val Tag: AggregateEventTag[HelloiEvent] = AggregateEventTag[HelloiEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends HelloiEvent
...
/**
  * This interface defines all the commands that the HelloiEntity supports.
  */
sealed trait HelloiCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends HelloiCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends HelloiCommand[String]
...
/**
  ...
  * application loader.
  */
object HelloiSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[HelloiState]
  )
}
