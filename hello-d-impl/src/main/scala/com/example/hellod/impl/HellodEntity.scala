...
/**
  * This is an event sourced entity. It has a state, [[HellodState]], which
  ...
  */
class HellodEntity extends PersistentEntity {

  override type Command = HellodCommand[_]
  override type Event = HellodEvent
  override type State = HellodState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: HellodState = HellodState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case HellodState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        HellodState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class HellodState(message: String, timestamp: String)
...
object HellodState {
  /**
    ...
    */
  implicit val format: Format[HellodState] = Json.format
}

/**
  * This interface defines all the events that the HellodEntity supports.
  */
sealed trait HellodEvent extends AggregateEvent[HellodEvent] {
  def aggregateTag: AggregateEventTag[HellodEvent] = HellodEvent.Tag
}

object HellodEvent {
  val Tag: AggregateEventTag[HellodEvent] = AggregateEventTag[HellodEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends HellodEvent
...
/**
  * This interface defines all the commands that the HellodEntity supports.
  */
sealed trait HellodCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends HellodCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends HellodCommand[String]
...
/**
  ...
  * application loader.
  */
object HellodSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[HellodState]
  )
}
